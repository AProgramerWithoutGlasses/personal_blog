...
func loginFrontHandler(c *gin.Context) {
	fmt.Println(1111)
	// 接收参数
	...
	if err != nil {
		...
		response.Fail(c, "index", response.ErrParamBind)
	...
	if err != nil {
		...
		response.Fail(c, "index", response.ErrServer)
	...
	response.SuccessWithMsg(c, "index", "登录成功", loginRes)
	fmt.Println(33333)
